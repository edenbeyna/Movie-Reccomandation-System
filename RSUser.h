...
typedef std::shared_ptr<RecommenderSystem> rec_system;

class RSUser
{
public:
  RSUser(const std::string& username, rank_map &user_rankings,
	 ...
	 */
     friend std::ostream &operator<<(std::ostream &output, const RSUser& user);

 private:
  std::string _username;
  rank_map _user_rankings;
  rec_system _recommendation;
};
